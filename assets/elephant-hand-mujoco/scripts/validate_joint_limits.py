...
CLOSED_BASELINE_MM = (
  87.3  # circumdiam through tip centers at the closed pose = 0 object
)
...
def main():
  ...
  for jn, (lo, hi) in V.JOINT_LIMITS.items():
    ...
    if jn in driven:
      ...
      cmatch = (
        abs(cr[0] - lo) < 1e-6
        and abs(cr[1] - hi) < 1e-6
        and bool(mjm.actuator_ctrllimited[ai])
      )
      ok &= cmatch
      extra = f" ctrlrange=[{cr[0]:+.3f},{cr[1]:+.3f}] {'OK' if cmatch else 'MISMATCH'}"
    print(
      f"  {jn:16s} range=[{r[0]:+.3f},{r[1]:+.3f}] lim={lim} {'OK' if match else 'MISMATCH'}{extra}",
      flush=True,
    )
  ...
  print(
    "\n=== (2) drive 6 driven joints to BOTH limits; finite + weld small ===",
    flush=True,
  )
  for which, idx in (("MIN", 0), ("MAX", 1)):
    ...
    ok &= passed
    print(
      f"  all driven -> {which} limit: finite={fin} weld_max={wmax:.3f} mm "
  ...
  open_ap = aps["OPEN(min,-0.70)"]
  closed_ap = aps["CLOSED(max,+0.90)"]
  ap_ok = abs(open_ap - 130) < 12 and abs(closed_ap - 0) < 6
  ok &= ap_ok
  print(
    f"  aperture spans ~0..130 mm: open={open_ap:.1f} (~130), closed={closed_ap:.1f} (~0) "
    f"-> {'PASS' if ap_ok else 'FAIL'}",
    flush=True,
  )
  ...
  try:
    ...
    n_lim = sum(
      int(mjm_c.jnt_limited[V._attached_id(mjm_c, mujoco.mjtObj.mjOBJ_JOINT, jn)])
  ...
  for jn in V.JOINT_LIMITS:
    ...
    if not (abs(abs(r[0]) - np.pi) < 1e-3 and abs(abs(r[1]) - np.pi) < 1e-3):
      ...
      print(
        f"  CHANGED: {jn} range=[{r[0]:+.3f},{r[1]:+.3f}] (expected +/-pi)", flush=True
      )
  print(
    f"  base build hand joints still at URDF +/-pi (derived limits NOT applied): "
    f"{still_placeholder}",
    flush=True,
  )
