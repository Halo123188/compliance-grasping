...
def sweep_curl(mjm, mjd, tg, lo=-1.6, hi=1.6, n=41, settle=150):
    ...
    for f in V.WELD_FINGERS:
        ...
        for direction in ("up", "down"):
            _reset_assembly(mjm, mjd)
            _settle(mjm, mjd, 150)
            seq = [t for t in targets if t >= inp0] if direction == "up" else \
                  [t for t in targets if t < inp0][::-1]
            for tgt in seq:
                mjd.ctrl[ai] = tgt
        ...
        print(f"\n  -- {f['name']} input (assembly={inp0:+.3f} rad = {inp0*R2D:+.1f} deg) --", flush=True)
        print(f"     {'input rad':>10s} {'input deg':>10s} {'aperture mm':>12s} {'weld mm':>9s} {'feasible':>9s}", flush=True)
...
def report_aperture_calibration(curl_res):
    ...
    for name, (inp0, rows) in curl_res.items():
        ...
        print(f"  {name}: feasible curl [{umin_feas:+.3f},{umax_feas:+.3f}] rad "
              f"([{umin_feas*R2D:+.1f},{umax_feas*R2D:+.1f}] deg); "
              f"aperture spans {amin:.1f}..{amax:.1f} mm", flush=True)
    return feas
...
def sweep_curl_sync(mjm, mjd, tg, lo=-0.9, hi=1.6, n=51, settle=180):
    ...
    rows = []
    print(f"     {'delta rad':>10s} {'aperture mm':>12s} {'weld mm':>9s} {'feasible':>9s}", flush=True)
    for d in deltas:
        _reset_assembly(mjm, mjd)
        _settle(mjm, mjd, 120)
        for f in V.WELD_FINGERS:
    ...
    print(f"\n  synchronous aperture range: {aps.min():.1f}..{aps.max():.1f} mm over "
          f"delta [{ds.min():+.3f},{ds.max():+.3f}] rad", flush=True)
    ...
    if aps.min() <= 130 <= aps.max():
        ...
        print(f"  130 mm not bracketed; closest aperture {aps.min():.1f}..{aps.max():.1f}", flush=True)
    dmin_ap = ds[np.argmin(aps)]
    print(f"  min aperture {aps.min():.1f} mm (most CLOSED) at curl delta = {dmin_ap:+.3f} rad", flush=True)
    return rows, inp0
...
def _finger_geom_sets(mjm):
    ...
    finger_bodies = {
        "F1": ["m_link_step", "s_link2_step", "s_link1_step", "finger1_step", "s_link2_step_2"],
        "F2": ["dae_mhl_step", "finger3_step", "finger2_step", "finger1_step_2", "finger3_step_2"],
        "F3": ["dae_mhr_step", "finger4_step", "finger2_step_2", "finger1_step_3", "finger4_step_2"],
    }
    ...
    tipg = {f["name"]: [V._attached_id(mjm, mujoco.mjtObj.mjOBJ_BODY, f["tip"])] for f in V.WELD_FINGERS}
    return fg, pg
...
def sweep_base(mjm, mjd, lo=-1.2, hi=1.2, n=49, settle=80, margin=0.002):
    ...
    print("\n===== 3. BASE swing range (selective self-collision, mesh-proximity) =====", flush=True)
    ...
    for f in V.WELD_FINGERS:
        ...
        for direction in ("up", "down"):
            _reset_assembly(mjm, mjd)
            _settle(mjm, mjd, 100)
            targets = np.linspace(base0, base0 + (hi if direction == "up" else lo), n // 2 + 1)
            for tgt in targets:
                mjd.ctrl[ai] = tgt
                for _ in range(settle):
                    mujoco.mj_step(mjm, mjd)
                ...
                wmax = max(al.values()) if al else 0.0
                collide = clr < margin
                rows.append((float(mjd.qpos[_jadr(mjm, f["base"])]), float(clr), bool(collide), wmax <= WELD_THRESH_MM))
                if collide:
                    break
        rows.sort(key=lambda r: r[0])
        res[name] = (base0, rows)
        print(f"\n  -- {name} base (assembly={base0:+.3f} rad = {base0*R2D:+.1f} deg), margin={margin*1000:.0f} mm --", flush=True)
        print(f"     {'base rad':>10s} {'base deg':>10s} {'clearance mm':>13s} {'collide':>8s} {'weldOK':>7s}", flush=True)
        for (b, clr, col, wok) in rows:
            clrs = f"{clr*1000:13.1f}" if np.isfinite(clr) else f"{'nan':>13s}"
            print(f"     {b:10.4f} {b*R2D:10.1f} {clrs} {str(col):>8s} {str(wok):>7s}", flush=True)
    return res
...
def sweep_passive_rom(mjm, mjd, curl_feas_band):
    ...
    for f in V.WELD_FINGERS:
        ...
        for p in passive:
            print(f"     {p:16s}: [{mins[p]:+.4f}, {maxs[p]:+.4f}] rad  "
                  f"([{mins[p]*R2D:+.1f}, {maxs[p]*R2D:+.1f}] deg)", flush=True)
    return rom
