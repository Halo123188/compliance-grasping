...
def settle_test(m, gravity=False, nsteps=300):
    ...
    for f in FINGERS:
        for jn in (f["base"], f["input"]):
            ai = mujoco.mj_name2id(m, mujoco.mjtObj.mjOBJ_ACTUATOR, f"act_{jn}".replace(" ", "_"))
...
def main():
    base = dict(
        mass_floor=0.04, inertia_floor=5e-5, armature=0.01, damping=0.3,
        solref=[0.01, 1.0], solimp=[0.99, 0.999, 0.001, 0.5, 2.0],
        impratio=10.0, timestep=0.001, iterations=150, ls_iterations=50,
        solver=mujoco.mjtSolver.mjSOL_NEWTON, kp=5.0, kv=0.5, torquescale=1.0,
    )
    ...
    print(f"baseline: finite={ok} max_weld_res={res:.4f} mm  (stopped at step {k})", flush=True)

    ok, res, d, k = settle_test(m, gravity=True)
    print(f"baseline w/GRAVITY: finite={ok} max_weld_res={res:.4f} mm (step {k})", flush=True)

    # sweep the key knobs
    print("\n=== sweep mass_floor x inertia_floor x armature (gravity ON, 300 steps) ===", flush=True)
    results = []
    for mf, inf, arm in itertools.product(
        ...
        print(f"  mass={mf:.2f} inertia={inf:.0e} arm={arm:.3f} -> {flag} res={res:.4f}mm step={k}", flush=True)
    ...
    m = build(chosen)
    print("config: mass_floor=0 inertia_floor=5e-5 armature=0.01 damping=0.3 "
          "solref=[0.01,1] solimp=[0.99,0.999,..] impratio=10 dt=0.001 Newton/150", flush=True)
    for f in FINGERS:
        ...
        print(f"  {f['name']}: input[{inputs[0]:+.2f}..{inputs[-1]:+.2f}] "
              f"anchor-align max={align.max():.4f} mean={align.mean():.4f} mm | "
              f"efc-res max={res.max():.4f} mean={res.mean():.4f} mm | finite={finite}", flush=True)
...
def sweep_input(m, f, gravity=True, lo=-0.6, hi=1.2, n=25, settle=60):
    ...
    ai_base = mujoco.mj_name2id(m, mujoco.mjtObj.mjOBJ_ACTUATOR, f"act_{base_jn}".replace(" ", "_"))
    ai_inp = mujoco.mj_name2id(m, mujoco.mjtObj.mjOBJ_ACTUATOR, f"act_{inp_jn}".replace(" ", "_"))
