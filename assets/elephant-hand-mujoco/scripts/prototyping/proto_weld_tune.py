...
def main():
    base = dict(
        mass_floor=0.0, inertia_floor=5e-5, armature=0.01, damping=0.3,
    ...
    for dmax in [0.999, 0.9999, 0.99999]:
        ...
        eval_cfg(dict(base, solref=[0.005, 1.0], impratio=20.0, solimp=si), label=f"dmax={dmax}")

    print("\n=== timestep sweep (best stiff config) ===", flush=True)
    bestish = dict(base, solref=[0.005, 1.0], impratio=20.0,
                   solimp=[0.99, 0.9999, 0.0005, 0.5, 2.0])
    ...
    print("\n=== minimal regularization: inertia_floor sweep w/ armature=0.01 (solref=0.005) ===", flush=True)
    stiff = dict(base, solref=[0.005, 1.0], solimp=[0.99, 0.999, 0.001, 0.5, 2.0],
                 impratio=1.0, armature=0.01, damping=0.3)
    for inf in [0.0, 1e-6, 5e-6, 1e-5, 5e-5]:
        eval_cfg(dict(stiff, inertia_floor=inf), label=f"inertia_floor={inf:.0e}")

    print("\n=== armature sweep with inertia_floor=0 (armature-only regularization) ===", flush=True)
    for arm in [0.005, 0.01, 0.02, 0.05]:
        eval_cfg(dict(stiff, inertia_floor=0.0, armature=arm), label=f"armature={arm} (inf=0)")
