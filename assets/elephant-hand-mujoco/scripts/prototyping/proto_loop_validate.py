...
def main():
  ...
  print(
    f"initial efc (after forward at assembly): nefc={d.nefc} "
  ...
  print(
    f"after 200 settle steps: finite={finite_settle} max|efc_pos|={settle_res:.2e} m",
    flush=True,
  )

  # ---- drive each finger across range, measure tip travel + residual ----
  print(
    "\n--- sweep driven joints, measure fingertip travel + weld residual ---",
    flush=True,
  )
  for f in FINGERS:
    reset_to(q0)
