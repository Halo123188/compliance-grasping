...
def body_rel(b1, b2):
    ...
    R1 = d.xmat[bid(b1)].reshape(3, 3); R2 = d.xmat[bid(b2)].reshape(3, 3)
    rel_R = R1.T @ R2; rel_p = R1.T @ (p2 - p1)
...
for f in FINGERS:
    ...
    for bv in base_grid:
        def resid(x):
            ...
            qd = np.zeros(4); mujoco.mju_mulQuat(qd, rq_inv, tquat)
            if qd[0] < 0: qd = -qd
    ...
    print(f"\n{f['name']} (base={f['base']}): {len(valid_base)} valid configs "
          f"over base in [{valid_base.min():.2f},{valid_base.max():.2f}]", flush=True)
    for j in passive:
        y = np.array(curves[j])
        ...
        print(f"   {j:16s} range[{y.min():+.3f},{y.max():+.3f}] "
