...
FINAL = dict(
    mass_floor=0.0, inertia_floor=0.0, armature=0.01, damping=0.3,
    solref=[0.002, 1.0], solimp=[0.99, 0.999, 0.001, 0.5, 2.0],
    impratio=1.0, timestep=0.001, iterations=150, ls_iterations=50,
...
def tip_track(m, f, drive_jn, lo, hi, n=25, settle=120):
    ...
    for _ in range(100):
        mujoco.mj_step(m, d)
    ai = mujoco.mj_name2id(m, mujoco.mjtObj.mjOBJ_ACTUATOR, f"act_{drive_jn}".replace(" ", "_"))
    tip = P.bid(m, f["tip"])
    b1, b2 = P.bid(m, f["b1"]), P.bid(m, f["b2"])
    pos_rel, _ = P.relpose(f["b1"], f["b2"])
    j0 = d.qpos[P.jadr(m, drive_jn)]
    sweep = np.linspace(j0 + lo, j0 + hi, n)
    tips, aligns, passive_ranges = [], [], {j: [] for j in f["loop"] if j not in (f["base"], f["input"])}
    finite = True
    for tgt in sweep:
        d.ctrl[ai] = tgt
...
def main():
    m = P.build(FINAL)
    print(f"FINAL config: armature={FINAL['armature']} damping={FINAL['damping']} "
    ...
    for f in P.FINGERS:
        ...
        print(f"  {f['name']}: net={net:.1f} path={path:.1f} mm (net/path={ratio:.3f}) "
              f"align_max={al:.4f}mm finite={fin}  passive_range[{pr_str}]", flush=True)

    print("\n=== drive BASE swing joint (independent DOF, weld must stay aligned) ===", flush=True)
    for f in P.FINGERS:
        net, path, al, pr, fin = tip_track(m, f, f["base"], -0.5, 0.5)
        print(f"  {f['name']}: base-swing tip net={net:.1f} mm align_max={al:.4f}mm finite={fin}", flush=True)
