...
for f in FINGERS:
    ...
    print(f"{f['name']}: loop angles (rad) = "
          + ", ".join(f"{j}={v:.4f}" for j, v in zip(f["loop"], x)), flush=True)
    print(f"   residual: pos_err={pos_err_mm:.3f} mm   rot_err={rot_err_deg:.3f} deg", flush=True)
