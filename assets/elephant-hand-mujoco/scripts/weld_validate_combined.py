...
def main():
    ...
    print(f"  solver=Newton iterations={mjm.opt.iterations} ls_iterations={mjm.opt.ls_iterations}"
          f" timestep={mjm.opt.timestep}", flush=True)
    print(f"  armature={V.WELD_ARMATURE} damping={V.WELD_DAMPING} "
          f"mass_floor={V.WELD_MASS_FLOOR} inertia_floor={V.WELD_INERTIA_FLOOR}", flush=True)
    print(f"  weld solref={V.WELD_SOLREF} solimp={V.WELD_SOLIMP}", flush=True)
    print(f"  nq={mjm.nq} nu={mjm.nu} neq={mjm.neq} (3 welds x 6 = nefc {3*6})", flush=True)
    ...
    print(f"  (b) max efc residual over sweep: {max_res*1000:.4f} mm "
          f"(force-weighted constraint stress during peak slew; not geometric distance)",
          flush=True)
    print(f"  (b) max coupler SEPARATION over sweep (per finger) -- peak | settled:", flush=True)
    for n in max_sep:
        print(f"        {n}: peak {max_sep[n]:.4f} mm | settled {max_sep_settled[n]:.4f} mm",
              flush=True)

    # --- (c) clean 1-DOF: drive one input slowly, confirm passive joints follow + tip smooth ---
    print("\n=== (c) clean 1-DOF articulation (drive F1 input, watch passive joints) ===", flush=True)
    ...
    passive_move = {p: np.rad2deg(mjd.qpos[_jadr(mjm, p)] - passive0[p]) for p in f["passive"]}
    print(f"  F1 tip net={net:.1f} mm path={path:.1f} mm (smooth curve, net<=path)", flush=True)
    print(f"  F1 passive joint motion over the drive:", flush=True)
