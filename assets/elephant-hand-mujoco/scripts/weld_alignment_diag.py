...
def main():
  print(
    "Loop-closure alignment-under-movement diagnostic (combined arm+hand).", flush=True
  )
  print(
    f"sweep: input in assembly+[{INPUT_LO},{INPUT_HI}] rad, {N_STEPS} pts, "
    f"{SETTLE} settle steps/pt, gravity ON, arm at pre-grasp.",
    flush=True,
  )
  ...
  print(
    ...
    f"Newton/{V.WELD_ITERATIONS} dt={V.WELD_TIMESTEP})",
    flush=True,
  )
  weld_res = sweep_model(
    mjm_w, mjd_w, "WELD — alignment per swept input", per_target_rows=True
  )

  # ---- MIMIC model (build_model_coupled) ----
  mjm_m, mjd_m = V.build_model_coupled()
  print(
    f"\nMIMIC model: nq={mjm_m.nq} nu={mjm_m.nu} neq={mjm_m.neq} (mjEQ_JOINT cubic)",
    flush=True,
  )
  ...
  # ---- Summary table ----
  print(
    "\n\n================ SUMMARY: coupler alignment under movement ================",
    flush=True,
  )
  print(
    f"{'Finger':6s} | {'WELD max':>9s} {'WELD mean':>10s} {'stable':>7s} | "
    f"{'MIMIC max':>10s} {'MIMIC mean':>11s} {'stable':>7s} | {'improvement':>12s}",
    flush=True,
  )
  print("-" * 86, flush=True)
  for f in V.WELD_FINGERS:
    ...
    print(
      f"{n:6s} | {wmax:8.4f}m {wmean:9.4f}m {str(wfin):>7s} | "
      f"{mmax:9.4f}m {mmean:10.4f}m {str(mfin):>7s} | {impr:10.1f}x tighter",
      flush=True,
    )
  print("-" * 86, flush=True)
  print(
    "(units mm; 'align' = world distance between the two duplicate coupler bodies'",
    flush=True,
  )
  print(
    " shared weld anchor. WELD physically fuses them -> ~0; MIMIC only joint-couples",
    flush=True,
  )
  print(
    " -> the two bodies drift apart = the 'two mounts appear' symptom.)", flush=True
  )
