...
HAND_URDF = (
  Path(__file__).resolve().parent / "assets/robots/mygripper_H100_R/robot.urdf"
)
...
def build_hand_only(closure: str = "weld") -> tuple[mujoco.MjModel, mujoco.MjData]:
  ...
  if closure == "coupled":
    ...
    for geom in hand.geoms:
      geom.contype = (
        0  # disable self-collision; keep conaffinity so compiler retains geoms
      )
...
def _print_stats(
  ...
  if welded:
    ...
    print(
      f"  Actuators : {mjm.nu}  (7 arm + 6 driven gripper DOFs; loop closed by weld)"
    )
    print(f"  Equalities: {mjm.neq}  (3 mjEQ_WELD four-bar loop closures)")
    print(
      f"  Stability : armature={WELD_ARMATURE} damping={WELD_DAMPING} "
      f"mass_floor={WELD_MASS_FLOOR} inertia_floor={WELD_INERTIA_FLOOR}"
    )
    print(
      f"  Solver    : Newton iters={WELD_ITERATIONS} ls={WELD_LS_ITERATIONS} "
      f"dt={WELD_TIMESTEP}  weld solref={WELD_SOLREF} solimp={WELD_SOLIMP}"
    )
  elif coupled:
    print(f"  DOF       : {mjm.nq}  (7 arm + 15 gripper joints + 6 object freejoint)")
    print(
      f"  Actuators : {mjm.nu}  (7 arm + 6 driven gripper DOFs; 9 loop joints slaved)"
    )
...
def main() -> None:
  ...
  class MyGripperViewer(mjviser.Viewer):
    ...
    def _setup_weld_panel(self) -> None:
      ...
      with gui.add_folder(
        "Weld Alignment (live) — should stay ~0 mm", expand_by_default=True
      ):
        gui.add_markdown(
    ...
    def _setup_mount_panel(self) -> None:
      ...
      gui = self._server.gui
      with gui.add_folder(
        "Mount Adjustment — gripper on flange", expand_by_default=True
      ):
        gui.add_markdown(
      ...
      def _apply(_=None) -> None:
        ...
        mujoco.mju_euler2Quat(
          dq, np.deg2rad([s_rx.value, s_ry.value, s_rz.value]), "xyz"
        )
        new_quat = np.zeros(4)
      ...
      def _print(_event) -> None:
        ...
        print(
          f"  mid = mujoco.mj_name2id(mjm, mujoco.mjtObj.mjOBJ_BODY, '{MOUNT_BODY}')"
        )
        print(f"  mjm.body_pos [mid] = np.array({pstr})")
        print(f"  mjm.body_quat[mid] = np.array({qstr})")
        print(
          f"  # euler deltas: rx={s_rx.value:.1f} ry={s_ry.value:.1f} rz={s_rz.value:.1f}\n"
        )

  if port_arg is not None:
    import viser

    viewer = MyGripperViewer(
      model=mjm, data=mjd, server=viser.ViserServer(port=port_arg)
    )
  else:
  ...
  print(
    "Drag sliders to orient the gripper, click Print to bake the values. Ctrl-C to stop.\n"
  )
  viewer.run()
