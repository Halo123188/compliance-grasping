...
def flexiv_two_finger_grasp_ppo_runner_cfg(
  ...
  """
  return _base("flexiv_two_finger_grasp", init_std, entropy_coef)


def flexiv_two_finger_vision_ppo_runner_cfg(
  experiment_name: str = "flexiv_two_finger_grasp_vision",
) -> RslRlOnPolicyRunnerCfg:
...
# 50 mm box in a 128x72 frame, and two stride-2 convs leave a receptive field
# too small to see a whole cube face. Three layers put the feature map at 16x9
# with 64 channels, so spatial softmax returns 128 keypoint coordinates.
_STUDENT_CNN_CFG = {
...
# Same trunk with the FIRST stride halved, so the map ends at 32x18 instead of
# 16x9. Motivated by where the student stalls rather than by theory: at 600
# iterations it lifts the cube to a 78.7 mm median but clears the 100 mm bar
# only 3.1% of the time, i.e. it finds the cube and misses on precision. The
# cube is 9-15 px wide in a 128x72 frame (`scripts/diag_student_view.py`), and
# three stride-2 layers collapse that to roughly ONE cell of the final map --
# so a keypoint can only place it to within ~8 input pixels. Quadrupling the
# output cells costs one layer's worth of compute and no parameters.
_STUDENT_CNN_FINE_CFG = {**_STUDENT_CNN_CFG, "stride": [1, 2, 2]}


def flexiv_two_finger_distill_runner_cfg(
  experiment_name: str = "flexiv_two_finger_grasp_distill",
