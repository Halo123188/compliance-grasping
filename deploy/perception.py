...
class RealSenseDepth:
  ...
  def set_preset(self, sensor, name: str) -> None:
    ...
    Matching is case-insensitive because the driver spells them `High_Density`
    and every CLI here documents them lowercase; a wrong preset is invisible in
    the depth image and would surface only as a policy behaving differently
    from the one that was tuned, so it must not be a silent no-op either.
    """
    name = name.lower()
