...
def test_sines_respect_both_the_position_window_and_the_speed_cap(joint):
  ...
  """
  inertia = float(np.array([3.80, 4.42, 2.10, 2.48, 0.19, 0.18, 0.13])[joint])
  tau_cap = 0.25 * float(EFFORT_LIMIT[joint])
  dq_cap = max(DQ_CAP_FLOOR, DQ_CAP_FRAC * float(DQ_MAX[joint]))
  x_max = 0.5 * Q_WINDOW
